...
public class LoginInterceptor implements HandlerInterceptor {
    public boolean preHandle(HttpServletRequest request,
        ...
        if (contextPath2.equals("course_range_sys") ||
                ((contextPath2.contains("admin") == false) &&
                (contextPath2.contains("teacher") == false) &&
                (contextPath2.contains("student") == false))) {
            return true;
        }
        ...
        if (login == null) {
            ...
            if (login.getLoginType() == 2) {
                if ((requestUri.contains("teacher") == false) &&
                        (requestUri.contains("commonapi") == false)) { //当前登录角色为教师,但请求其他角色使用的接口,即盗链,这种需要拦截并提示错误
                    response.sendRedirect(contextPath +
                        "/commonapi/error?msg=-2");
            ...
            if (login.getLoginType() == 3) {
                if ((requestUri.contains("student") == false) &&
                        (requestUri.contains("commonapi") == false)) { //当前登录角色为学生,但请求其他角色使用的接口,即盗链,这种需要拦截并提示错误
                    response.sendRedirect(contextPath +
                        "/commonapi/error?msg=-2");

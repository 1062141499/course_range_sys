    ...
    */
    ...
    public Object sysSubmit(LoginModel user, String imgCode, ModelMap modelMap,
        ...
        if (user.getLoginType() == 2) {
            TeacherInfoExample te = new TeacherInfoExample(); //验证教师账号密码
            TeacherInfoExample.Criteria tc = te.createCriteria();
            tc.andTnoEqualTo(user.getName());
            tc.andPassWordEqualTo(user.getPassword());

            List<TeacherInfo> tl = teacherInfoMapper.selectByExample(te);
        ...
        if (user.getLoginType() == 3) {
            StudentInfoExample te = new StudentInfoExample(); //验证学生账号密码
            StudentInfoExample.Criteria tc = te.createCriteria();
            tc.andStuNoEqualTo(user.getName());
            tc.andPasswordEqualTo(user.getPassword());

            List<StudentInfo> tl = studentInfoMapper.selectByExample(te);

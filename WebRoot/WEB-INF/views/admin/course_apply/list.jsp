												...
												<fieldset class="layui-elem-field">
													...
													<div class="layui-field-box">
														<div>
															...
															<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=2646"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/course_apply/queryList",
		data : {
			"page":page,
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				if(rows[i].courseApply.applyStatus=='1'){
					html+='<a href="${pageContext.request.contextPath}/admin/course_apply/tg?id='+rows[i].courseApply.id+'" class="layui-btn layui-btn-radius btnStyle">申请通过</a>';
				}
				if(rows[i].courseApply.applyStatus=='1'){
					html+='<a href="${pageContext.request.contextPath}/admin/course_apply/btg?id='+rows[i].courseApply.id+'" class="layui-btn layui-btn-danger layui-btn-radius btnStyle">申请不通过</a>';
				}
				html+='<a href="javascript:void(0)" onclick="del2(this)" data-id="'+rows[i].courseApply.id+'" class="layui-btn layui-btn-danger layui-btn-radius btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function del2(e){
	if(window.confirm('你确定要删除该调停课申请吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/course_apply/del2",
			data : {
				"id":id
			},
			success : function(result) {
